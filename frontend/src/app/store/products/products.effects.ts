...
export class ProductsEffects {
  ...
  // ── Trigger fresh load when search term changes ───────────────────
  ...
  // ── Fresh load — replaces product list ───────────────────────────
  ...
  loadMoreProducts$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatLatestFrom(() => [
        ...
        this.store.select(selectActiveCategory),
        this.store.select(selectSortBy),
        this.store.select(selectPage),        // current page already in store
        this.store.select(selectPageSize),
        this.store.select(selectHasMore),     // checked BEFORE reducer sets loadingMore
      ]),
      // ✅ Skip if nothing more to load
      filter(([, , , , , , hasMore]) => !!hasMore),
      // ✅ exhaustMap: ignores new dispatches while a request is in flight
